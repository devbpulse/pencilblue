...
class RouteService {
    ...
    /**
     * Retrieves the static route for the provided URL path
     * @returns {object}
     */
    ...
    static getRoute (path, siteUid) {
        ...
        var route = staticRoutes[path];
        if (!_.isNil(route)) {
            if (route.themes[siteUid] || route.themes[SiteUtils.GLOBAL_SITE]) {
                if (isSilly) {
                    log.silly('Found static route [%s]', path);
                }
                return route;
            }
        }
        ...
        for (var i = 0; i < storage.length; i++) {
            ...
            if (isSilly) {
                log.silly('Comparing Path [%s] to Pattern [%s] Result [%s]', path, curr.pattern, result);
            }
            if (result) {
     ...
     * @param {String} theme The plugin/theme uid
     * @param {string} site
     * @return {number} The number of routes removed
     */
    static unregisterTheme (theme, site) {
        var routesRemoved = 0;

        //pattern routes
        for (var i = 0; i < storage.length; i++) {
            var pathStr = storage[i].path;
            var result = RouteService.unregisterRoute(pathStr, theme, site);
            if (result) {
                routesRemoved++;
            }
        }
        ...
        //static routes
        Object.keys(staticRoutes).forEach(function(pathStr) {
            var result = RouteService.unregisterRoute(pathStr, theme, site);
            if (result) {
                routesRemoved++;
            }
