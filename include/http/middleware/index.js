...
const SessionHandler = require('../../session/session');console.log('mw-sh', SessionHandler);
...
class Middleware {
    ...
    static principal (req, res, next) {
        ...
        req.headers[SessionHandler.COOKIE_HEADER] = cookies;

        //open session
        req.handler.sessionHandler.open(req, function (err, session) {
            if (_.isError(err)) {
                return next(err);
            }
            if (!session) {
    ...
    static deriveSite (req, res, next) {
        ...
        if (!siteObj) {
            return next(new Error("The host (" + hostname + ") has not been registered with a site. In single site mode, you must use your site root (" + Configuration.active.siteRoot + ")."));
        }
    ...
    static deriveActiveTheme (req, res, next) {
        var settings = SettingServiceFactory.getService(Configuration.active.settings.use_memory, Configuration.active.settings.use_cache, req.siteObj.uid);
        settings.get('active_theme', function (err, activeTheme) {
            if (_.isError(err)) {
                return next(err);
            }
            if (!activeTheme) {
                log.warn("RequestHandler: The active theme is not set.  Defaulting to '%s'", Configuration.active.plugins.default);
                activeTheme = Configuration.active.plugins.default;
            }
    ...
    static deriveRouteTheme (req, res, next) {
        ...
        if (log.isSilly()) {
            log.silly("RequestHandler: Settling on theme [%s] and method [%s] for URL=[%s:%s]", rt.theme, rt.method, req.method, req.handler.url.href);
        }
    ...
    static authorizationCheck (req, res, next) {
        ...
        if (!result.success && log.isDebug()) {
            log.debug('AuthCheck: %s', result.message);
        }
    ...
    static writeSessionCookie (req, res, next) {
        ...
        if (req.setSessionCookie) {
            try {
                cookies.set(SessionHandler.COOKIE_NAME, req.session.uid, SessionHandler.getSessionCookie(req.session));
            }
            catch (e) {
                log.error('RequestHandler: Failed to set cookie: %s', e.stack);
            }
        }
        next();
    }
    ...
    static responseTime (req, res, next) {
        ...
        if (log.isDebug()) {
            log.debug("Response Time: %sms URL=%s%s%s",
    ...
    static principalClose (req, res, next) {
        ...
        if (req.session) {
            req.handler.sessionHandler.close(req.session, function (err/*, result*/) {
                if (_.isError(err)) {
                    log.warn('RequestHandler: Failed to close session [%s]', req.session.uid);
                }
            });
        }
        next();
