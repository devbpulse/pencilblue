...
MemoryEntityService.prototype.onSet = function(key, value) {
    ...
    CommandService.getInstance()
        .sendCommandToAllGetResponses(MemoryEntityService.getOnChangeType(), command, function(){});
};
...
MemoryEntityService.changeHandler = function(command) {
    ...
    memoryEntityService._set(command.key, command.value, function(){});
};
...
MemoryEntityService.reap = function() {
    ...
    log.silly('MemoryEntityService: Scanned %s keys and reaped %s in %sms', keys.length, reaped, Date.now() - now);
    return reaped;
};
...
MemoryEntityService.startReaper = function() {
    if (REAPER_HANDLE === null) {
        ...
        System.registerShutdownHook('MemoryEntityService', MemoryEntityService.dispose);

        log.silly('MemoryEntityService: Reaper started');
        return true;
    }
    return false;
};
