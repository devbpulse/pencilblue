...
var log = require('../utils/logging').newInstance('SimpleLayeredService');
...
SimpleLayeredService.prototype.get = function(key, cb){
    ...
    async.whilst(
        ...
        function(callback) {//do
            if (log.isSilly()) {
                log.silly('%s: Checking Service [%s] for key [%s]', instance.name, instance.services[i].type, key);
            }
            ...
            if (entity) {
                ...
                for (var j = 0; j < i; j++) {
                    if (instance.services[j]._set) {
                        instance.services[j]._set(key, entity, function() {});
                    }
                    else {
                       instance.services[j].set(key, entity, function() {});
                    }
                }
            }
    ...
    for (var i = this.services.length -1; i >= 0; i--) {
        var task = function(index){
            return function(callback) {
                if (log.isSilly()) {
                    log.silly("%s:%s: Setting [%s] Value %s", self.name, self.services[index].type, key, JSON.stringify(value));
                }
...
SimpleLayeredService.prototype.purge = function(key, cb){
    var tasks = this.services.map(function(service) {
        return function(callback) {
            service.purge(key, callback);
        };
    });
    async.series(tasks, cb);
};

module.exports = SimpleLayeredService;
