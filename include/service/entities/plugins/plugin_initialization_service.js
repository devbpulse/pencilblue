...
class PluginInitializationService {
    ...
    static initAll (cb) {
        log.debug('Beginning plugin initialization...');

        //reset the cache in case it gets called twice for some unexpected reason
        PLUGIN_INIT_CACHE = {};

        var self = this;
        async.waterfall([
            PluginRepository.loadPluginsAcrossAllSites,
                    ...
                    return function (callback) {
                        ...
                        try {
                            var service = new PluginInitializationService({
                                pluginService: new PluginService({ site: plugin.site }),
                                pluginCache: PLUGIN_INIT_CACHE
                            });
                            service.initialize(plugin, {}, function (err, didInitialize) {

                                //TODO [1.0] do we really need to reset the cache after each one?
                                PLUGIN_INIT_CACHE = {};
                                callback(null, {plugin: plugin, error: null, initialized: didInitialize});
                            });
                        }
                        catch (err) {
                            callback(null, {plugin: plugin, error: err, initialized: false});
                        }
                    };
                });
