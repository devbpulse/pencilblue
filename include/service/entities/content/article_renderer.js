...
var MediaLoader = require('../article_service').MediaLoader;
...
var UrlUtils = require('../../../../lib/utils/urlUtils');
    ...
    class ArticleRenderer {
        constructor(context) {
             ...
             * @type {string}
             */
            this.site = SiteService.getCurrentSite(context.site);
             ...
             * @type {UserService}
             */
            this.userService = context.userService || new UserService(context);
        }
         ...
         * @type {String}
         */
        static get READ_MORE_FLAG() {
            return '^read_more^';
        }

        /**
         * TODO [1.0] remove in favor of localization key iff really needed
         * @deprecated
         * @readonly
         * @type {String}
         */
        static get ANONYMOUS_COMMENTER() {
            return 'Anonymous';
        }
         ...
         * @param {Object} context
         */
        formatBylines(content, context) {

            var author = context.authors[content.author];
            if (_.isNil(author)) {
                log.warn('ArticleRenderer: Failed to find author [%s] for content [%s]', content.author, content[DAO.getIdField()]);
                return;
            }
         ...
         * @param {Object} context
         */
        formatTimestamp(content, context) {
            if (context.contentSettings.display_timestamp) {
                content.timestamp = ContentService.getTimestampTextFromSettings(
         ...
         * @param {Function} cb
         */
        formatMediaReferences(content, context, cb) {
            var self = this;
        ...
        formatComments(content, context, cb) {
            ...
            var opts = {
                where: {
                    article: content[DAO.getIdField()] + ''
                },
         ...
         */
        getCommenters(comments, contentSettings, cb) {
            var self = this;

            //callback for iteration to handle setting the commenter attributes
            var processComment = function (comment, commenter) {
                comment.commenter_name = ArticleRenderer.ANONYMOUS_COMMENTER;
                comment.timestamp = ContentService.getTimestampTextFromSettings(comment.created, contentSettings);
            ...
            var users = {};
            var tasks = TaskUtils.getTasks(comments, function (comment) {
                return function (callback) {
                    ...
                    dao.loadById(comment.commenter, 'user', function (err, commenter) {
                        ...
                        //process the comment
                        users[commenter[DAO.getIdField()].toString()] = commenter;
         ...
         * @param {Object} context
         */
        formatAutoBreaks(content, context) {
            var contentSettings = context.contentSettings;
            var breakString = '<br>';
            var tempLayout;
         ...
         */
        formatLayoutForReadMore(content, context) {
            var layout = this.getLayout(content);

            if (context.readMore) {
                var beforeReadMore = layout.substr(0, layout.indexOf(ArticleRenderer.READ_MORE_FLAG));
                layout = beforeReadMore + this.getReadMoreSpan(content, context.contentSettings.read_more_text);
            }
            else {
                layout = layout.split(ArticleRenderer.READ_MORE_FLAG).join('');
            }
         ...
         * @return {String}
         */
        getReadMoreSpan(content, anchorContent) {
         ...
         * @return {String}
         */
        getReadMoreLink(content, anchorContent) {

            var path = UrlUtils.join(this.getContentLinkPrefix() + content.url);
            return '<a href="' + UrlUtils.createSystemUrl(path, {hostname: this.hostname}) + '">' + anchorContent + '</a>';
        }
         ...
         * @return {Boolean}
         */
        containsReadMoreFlag(content) {
            if (typeof content !== 'object') {
                throw new Error('The content parameter must be an object');
            }
            return this.getLayout(content).indexOf(ArticleRenderer.READ_MORE_FLAG) > -1;
        }
    }
